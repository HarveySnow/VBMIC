...
# 导入了AdaptiveMultiCameraImageDataset,可以动态选择训练使用的摄像头数量。
...
os.environ["WANDB_API_KEY"] = "123456abcdefg" # write your own wandb id

'''
导入了AdaptiveMultiCameraImageDataset,可以动态选择训练使用的摄像头数量。
模型改为Multi_LDMIC,支持多摄像头的训练。
损失函数改为Multi_MSE_Loss和Multi_MS_SSIM_Loss。
训练过程中通过dataset.set_num_camera()动态设置使用的摄像头数。
去掉了单独的左右相机的bpp和指标统计,改为整体统计。
增加了一个命令行参数args.num_camera指定摄像头数量。
'''
...
def parse_args(argv):
    ...
    parser.add_argument(
        "-d", "--dataset", type=str, default='/data/Instereo2K/', help="Training dataset"
    )
    parser.add_argument(
        "--data-name", type=str, default='instereo2K', help="Training dataset"
    )
    # 模型改为Multi_VBMIC,支持多摄像头的训练。
    parser.add_argument(
        "--model-name", type=str, default='Multi_VBMIC', help="Training dataset"
    )
    parser.add_argument(
...
def main(argv):
    ...
    if args.model_name == "Multi_VBMIC":
        net = Multi_VBMIC(N=192, M=192, decode_atten=Multi_JointContextTransfer,)
    elif args.model_name == "Multi_VBMIC_checkboard":
        net = Multi_VBMIC_checkboard(N=192, M=192, decode_atten=Multi_JointContextTransfer,)
    ...
    project_name = "MultiCamera_VBMIC_" + args.data_name
