...
'''
区别
支持多视角图像,collect_images函数可以收集多相机的数据。
模型构建时指定为多视角版本,如Multi_LDMIC。
eval_model_entropy_estimation函数处理多视角时,对每个视角计算指标,最后取平均。
run_inference输入为多视角图像列表。
创建参数解析器时,添加了--num-camera参数,用于指定相机数量。
主函数中,filepaths变为包含多相机图像路径的列表。
评估时,IFrameCompressor输入变为图像列表,自动适配多视角输入。
'''
...
def main(args: Any = None) -> None:
    ...
    if args.IFrameModel == "Multi_VBMIC":
        IFrameCompressor = Multi_VBMIC(N=192, M=192, decode_atten=Multi_JointContextTransfer,)
    elif args.IFrameModel == "Multi_VBMIC_checkboard":
        IFrameCompressor = Multi_VBMIC_checkboard(N=192, M=192, decode_atten=Multi_JointContextTransfer,)
